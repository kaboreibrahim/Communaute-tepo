from Apps.person.dtos.person_dto import PersonDTO, UpdatePersonDTO
...
class UpdatePersonUseCase:
    """Modifie une personne. Memes regles que la creation."""
    ...
    def execute(self, data: UpdatePersonDTO) -> PersonDTO:
        existante = self.person_repo.get_by_id(data.id)
        if not existante:
            raise PersonIntrouvableError(
                f"Personne {data.id} introuvable."
            )
        ...
        if data.conjoint_id:
            ...
            if conjoint.conjoint_id and conjoint.conjoint_id != data.id:
        ...
        return self.person_repo.update(data)
